...
**QueueLogger** (`logging_setup.py`) is the thread-boundary contract. It puts typed tuples onto the queue: `("log", str)`, `("progress", dict)`, `("done", dict)`. The GUI's `_handle_progress` and `_handle_done` methods consume these. The GUI adds a fourth type, `("batch", {"index", "total", "name"})`, which only sets the `[i/n] name` prefix on the progress label.

**Batch queues:** both tabs hold a queue listbox (`queue_list`, `summary_queue_list`) fed by drag-and-drop, multi-select in `FileBrowserDialog` (`dialog.results`), or the entry box, which `_collect_queue` flushes into the list at Start. Items are processed sequentially in one worker thread; a single file is just a batch of one. Since `transcribe_file` calls `logger.done(...)` itself and `_handle_done` treats a `done` as the end of the whole run, the batch loop uses `_BatchLogger` (`gui.py`) — a `QueueLogger` subclass that turns each per-file `done()` into a log line and records `last_success` — and puts the one real `("done", ...)` on the queue itself after the loop. A failing item is logged and the batch continues; Stop (`_stop_event`) aborts the whole batch. All Tk vars are frozen into an `opts` dict before the thread starts.
...
**Output files** are written alongside the input (or to the configured output directory): `<name>_transcript.txt` (plain text) and `<name>_subtitles.srt` (timed subtitles), both prefixed with speaker labels when diarization is enabled. Summaries are auto-written as `<name>_summary.md` next to their source transcript (single runs too); the Summarise tab's "Save Summary" button is a manual Save-As of the displayed summary. Output names derive from the input stem only, so same-named inputs from different folders overwrite each other in a shared output directory — `_validate_queue` logs a warning when a batch contains duplicate stems.
